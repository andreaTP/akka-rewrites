...
import impl.CompilerSupport

/** same as [[scalafix.internal.rule.CompilerTypePrinter]] but:
  * + Share `unit: g.CompilationUnit` from `CompilerSupport`
  * + Always `rewriteStructuralTypesToNamedSubclass` */
class CompilerTypePrinter(val g: ScalafixGlobal)(
  implicit val doc: v1.SemanticDocument
) extends TypePrinter with CompilerSupport {
  import g._
  ...
  private def rewriteRefinedTypeToNamedSubclass(
    ...
    case resultType @ RefinedType(parents, decls)
        if /* config.rewriteStructuralTypesToNamedSubclass && */ //always rewrite
